...
class RBF(Network):
    ...
    def fit(self, inputs, labels, use_gradient):
        ...
        return self.__run_minimization(inputs, labels, omega, gradient=use_gradient)
    ...
    def compute_gradients(self, omega, inputs, labels):
        self.V = omega[0:self.hidden_size].reshape(self.hidden_size, 1)
        self.C = omega[self.hidden_size:].reshape(self.hidden_size,
                                                  self.input_size)
        c = np.tile(self.C.reshape(-1), inputs.shape[0])
        x = np.tile(inputs, self.hidden_size).reshape(-1)
        cx = (c - x).reshape(inputs.shape[0], self.hidden_size, 2) ** 2
        col = np.exp(-(cx[:, :, 0] + cx[:, :, 1]) / (self.sigma ** 2))
        # FIXME: SHAPE ERRORs (210, 1) vs (210, 25)
        var = (np.dot(col, self.V).reshape(-1, 1)) - labels
        dedv = (np.dot(var, col) / inputs.shape[0] + 2 * self.rho * self.V.T).reshape(-1, 1)

        _first_comp = (-(c - x)
                       ).reshape(inputs.shape[0], self.hidden_size, 2)[:, :, 0]
        _first_comp = 2 * (col * self.V.T * _first_comp) / (self.sigma ** 2)
        _var = np.dot(col, self.V).reshape(-1, 1) - labels
        first_comp = np.dot(_var, _first_comp) / inputs.shape[0]

        _scnd_comp = (-(c - x)
                      ).reshape(inputs.shape[0], self.hidden_size, 2)[:, :, 1]
        _scnd_comp = 2 * (col * self.V.T * _scnd_comp) / (self.sigma ** 2)
        _var = (np.dot(col, self.V)).reshape(-1, 1) - labels
        second_comp = np.dot(_var, _scnd_comp) / inputs.shape[0]

        dedc = np.append(first_comp.T, second_comp.T,
                         axis=1) + 2 * self.rho * self.C

        return np.concatenate(dedv.reshape(1, -1), dedc.reshape(1, -1), axis=1).reshape(-1)
